...
def power(sat):
    ...
    EFFICIENCY = 0.27
    INTENSITY = 1346                                         #in watts per metre square
    MIN_ANGLE = 90
    ...
    for i in range (0,6):
        effective_area=np.append(effective_area,[(0.0064)*cos_values[i]])    #area of each side is 0.01 msq
    v_POWER= np.array([])
    POWER=0
    for i in range (0,6):
        v_POWER = np.append(v_POWER,[EFFICIENCY*INTENSITY*effective_area[i]])
        POWER=POWER+EFFICIENCY*INTENSITY*effective_area[i]
    if (LIGHT==0 or LIGHT==0.5):
        v_POWER=np.array([0,0,0,0,0,0])
        POWER=0
    v_POWER=np.append(v_POWER,[POWER])
    return v_POWER